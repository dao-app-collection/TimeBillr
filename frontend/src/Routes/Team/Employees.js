...
const Employees = () => {
    ...
    useEffect(() => {
        ...
        if(orgContext.organizationData.TeamMemberships){
            setOwner(orgContext.organizationData.TeamMemberships.filter(member => {
                return member.permissions === 'owner'
            }));
            setManagers(orgContext.organizationData.TeamMemberships.filter(member => {
                return member.permissions === 'manager'
            }));
            setEmployees(orgContext.organizationData.TeamMemberships.filter(member => {
            ...
            <List
                ...
                renderItem={item => (
                    <List.Item>
                        ...
                        />
                        <Button type='primary' danger style={{backgroundColor: 'green'}} onClick={(e) => {ToggleModal(e, item)}}>
                            Edit Employee
                        </Button>
                    </List.Item>
                )}
    ...
    return (
        ...
        <CenteredContainer style={{maxWidth: '800px'}}>
            <Collapse accordian>
                ...
                <Panel header='Managers'>
                    {renderEmployeeList(managers)}
                </Panel>
                <Panel header='Owners'>
                    {renderEmployeeList(owner)}
                </Panel>
            </Collapse>
        </CenteredContainer>
        <EditEmployeeModal onSubmit={handleEditSubmit} employee={activeEmployee} open={editEmployee} toggle={ToggleModal} />
        </>
    );
};

const EditEmployeeModal = ({onSubmit, employee, open, toggle, form, sending}) => {
    ...
    if(open){
        return (
            ...
            onCancel={toggle}
            footer={[
                <ButtonWithSpinner sending={sending} form={'edit-employee'} innerHtml={'Submit'} submittable={submittable}/>
            ]}
