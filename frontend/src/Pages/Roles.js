...
const Roles = () => {
  ...
  if (orgContext.organizationData.TeamRoles) {
    rolesComponents = (
      <Role rolesArray={orgContext.organizationData.TeamRoles} />
    );
  }
  return (
    <CenteredContainer style={{ width: "800px" }}>
      <Menu mode="horizontal" onClick={toggleModal}>
        ...
        <Panel header={role.title}>
          ...
          <div style={{ display: "flex" }}>
            ...
            <Button
              ...
              style={ButtonStyle}
              onClick={(e) => {
                toggleConfirmationModal(e, role);
              }}
            >
              Delete
            </Button>
              ...
              <List
                ...
                renderItem={(item) => (
                  <List.Item>
                    <List.Item.Meta
                      title={
                        item.TeamMembership.User.firstName +
                        " " +
                        item.TeamMembership.User.lastName
                      }
                    />
...
const AddEmployeeToRole = ({ open, role, close }) => {
  ...
  const submit = async (values) => {
    const user = autoCompleteOptions.filter((employee) => {
      return employee.value === values.name;
    })[0];
    console.log(user);
    setSending(true);
    console.log(teamId);
  ...
  const createOptions = () => {
    ...
    let candidatesArray = orgContext.organizationData.TeamMemberships.filter(
      (membership) => {
        return (
          membership.EmployeeRoles.filter((employeeRoles) => {
